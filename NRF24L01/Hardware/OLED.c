...
void OLED_W_SCL( uint8_t BitVal)
{
	...
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_12, (GPIO_PinState)BitVal);
	//Delay_us(1);
}
void OLED_W_SDA(uint8_t BitVal)
{
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, (GPIO_PinState)BitVal);
	//Delay_us(1);
} 
...
void OLED_ShowFloatNum(uint8_t Line, uint8_t Column, double Number, uint8_t IntLength, uint8_t FraLength)
{
	...
	/*显示整数部分*/
	OLED_ShowNum(Line, Column+1, Number, IntLength);
	...
	OLED_ShowNum(Line,Column+IntLength+2, ((uint32_t)(Number * Temp)) % Temp, FraLength);
}
